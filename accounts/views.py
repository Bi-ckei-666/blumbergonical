...
def my_orders(request):
    ...
    return render(request, 'mainapp/dashboard/my_orders.html', context)
...
def forget_password(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('mainapp/forget_password/send_resetpassword_email.html', {
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),
                'token': account_activation_token.make_token(user),
    ...
    return render(request, 'mainapp/forget_password/forget_password.html') 
...
def reset_password(request):
    if request.method == 'POST':
        ...
        return render(request, 'mainapp/forget_password/reset_password.html')